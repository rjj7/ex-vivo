...
from dipy.direction.peaks import peak_directions, reshape_peaks_for_visualization
...
parser = argparse.ArgumentParser(
        description='Computing Diffusion Spectrum Imaging (DSI) - custom version')
...
parser.add_argument('--filter_type', dest='filter_type', action='store', default='none', help='Type of q-space filter to use, can be none, hanning, hamming. Default=none')
parser.add_argument('--filter_width', dest='filter_width', type=int, action='store', default=32, help='Width of Hanning window, only valid if filter_type is not none, int. Default=32')

parser.add_argument('--r_start', dest='r_start', type=float, action='store', default=2.1, help='Radial start point of ODF Sampling, float. Default=2.1')
parser.add_argument('--r_end', dest='r_end', type=float, action='store', default=6.0,help='Radial end point of ODF sampling, float. Default=6')
parser.add_argument('--r_step', dest='r_step', type=float, action='store', default=0.1,help='Step size of ODF sampling, float. Default=.1')
parser.add_argument('--r_weight', dest='r_weight', type=int, action='store', default=2,help='Radial weighting exponent, int. Default=2')
parser.add_argument('--qgrid_size', dest='qgrid_size', type=int, action='store', default=17, help='Sets the size of the q_space grid, int. Default=17')

parser.add_argument('--npeaks', dest='npeaks', action='store', default=3, help='Number of peaks to be selected, int. Default=3')
parser.add_argument('--rel_peak_thr', dest='rel_peak_thr', type=float, action='store', default=0.5, help='Relative peak threshold, float. Default=.5')
parser.add_argument('--min_sep_angle', dest='min_sep_angle',type=int, action='store',  default=25, help='Minimum separation angle, int. Default=25')
...
if args.r_weight:
	r_weight = np.asarray(args.r_weight, dtype='int')
else:
	r_weight = 2
...
print(" - r_start: " + str(r_start))
print(" - r_end: " + str(r_end))
print(" - r_step: " + str(r_step))
print(" - r_weight: " + str(r_weight))
print(" ---------------------------- ")
...
###### changed this part (rob 10/27/21)
...
print("Getting model...");
dsmodel = dsi.DiffusionSpectrumModel( gtab, filter_type=filter_type, filter_width=filter_width,
                                            qgrid_size=qgrid_size, r_start=r_start, r_end=r_end, pdfwgt=r_weight,
                                            r_step=r_step, normalize_peaks=norm_peaks );

# Fit custom DSI model to data
print("Getting fit...");
dsfit = dsmodel.fit( data, mask );

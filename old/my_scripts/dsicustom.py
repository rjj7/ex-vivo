...
class DiffusionSpectrumFit(OdfFit):
    ...
    def rtop_signal(self, filtering=False):
    ...
    def pdf_weighted(self, pdfweight=2):
        ...
        y = np.tile(a.reshape(gridsize, 1), (gridsize, 1, gridsize))
        z = np.tile(a.reshape(gridsize, 1, 1), (1, gridsize, gridsize))
        r2 = x ** rweight + y ** rweight + z ** rweight
...
class DiffusionSpectrumDeconvModel(DiffusionSpectrumModel):
    ...
    def __init__(self, gtab, qgrid_size=17, r_start=2.1, r_end=6., r_step=0.2, filter_width=32, filter_type='hanning', normalize_peaks=False, pdfwgt=2):

        DiffusionSpectrumModel.__init__(self, gtab, qgrid_size,
                                        r_start, r_end, r_step,
                                        filter_width, filter_type,
                                        normalize_peaks, pdfwgt)
